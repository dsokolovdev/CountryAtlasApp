...
final class LearnStatsCell: UICollectionViewCell {
    ...
    private let learnedProgressLabel: UILabel = {
        let lbl = UILabel()
        lbl.font = .rounded(ofSize: 22.scaled, weight: .bold)
    ...
    private let dashLabel: UILabel = {
        let lbl = UILabel()
        lbl.font = .rounded(ofSize: 20.scaled, weight: .regular)
    ...
    private let percentLearnedLabel: UILabel = {
        let lbl = UILabel()
        lbl.text = "%"
        lbl.font = .rounded(ofSize: 16.scaled, weight: .semibold)
        lbl.textAlignment = .left
        return lbl
    }()
    ...
    override init(frame: CGRect) {
        ...
        let width: CGFloat = 200.scaled
        let height: CGFloat = 10.scaled
    ...
    func configure(with stats: ContinentStats) {
        ...
        learnedProgress > 0
        ? .wildGreen
        : .wildGreen.withAlphaComponent(0.5)
        
        toLearnProgressLabel.textColor =
        toLearnProgress < 1
        ? .brightYellow
        : .brightYellow.withAlphaComponent(0.5)
        
        percentLearnedLabel.textColor =
        learnedProgress > 0
        ? .wildGreen.withAlphaComponent(0.8)
        : .wildGreen.withAlphaComponent(0.5)
        
        percentToLearnLabel.textColor =
        toLearnProgress < 1
        ? .brightYellow.withAlphaComponent(0.8)
        : .brightYellow.withAlphaComponent(0.5)
        
        dashLabel.textColor =
        learnedProgress > 0
        ? .lightBlueBallerina
        : .lightBlueBallerina.withAlphaComponent(0.5)
